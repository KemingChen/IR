...
class Evaluation:
	...
	def update(self, query_class='', calculated_class=''):
		# if self.confusion_matrix.has_key(query_class):
		# 	if self.confusion_matrix[query_class].has_key(calculated_class):
		# 		self.confusion_matrix[query_class][calculated_class] += 1
		# 	else:
		# 		print "error"
		# else:
		# 	print "error"
		
		if not self.confusion_matrix.has_key(query_class):
			query_class = "error"
		if not self.confusion_matrix[query_class].has_key(calculated_class):
			calculated_class = "error"
		self.confusion_matrix[query_class][calculated_class] += 1
	...
	def calculate(self, k=0, review_spam=False):
		...
		for i in self.confusion_matrix:
			...
			result[i]["fn"] = fn
			try:
				result[i]["precision"] = precision = tp/float(tp+fp)
				result[i]["recall"] = recall = tp/float(tp+fn) 
				result[i]["f_measure"] = 2*precision*recall/(precision+recall)
				result[i]["accuracy"] = float(tp + tn)/float(tp+tn+fp+fn)
...
class Rocchio(Index):	
	...
	def calculate_proto_classes(self):
		...
		sum_vector = SuperList()
		print "start calculate vector space"
		for doc in self.matrix:
		...
		print "bulid confusion matrix"
		confusion_matrix = {}
		confusion_matrix["error"] = {"error":0}
		for p_class in self.proto_classes.keys():
			confusion_matrix[p_class] = {};
			confusion_matrix["error"][p_class] = 0
			confusion_matrix[p_class]["error"] = 0
			for a_class in self.proto_classes.keys():
				confusion_matrix[p_class][a_class] = 0
		self.ev.confusion_matrix = confusion_matrix
		print "topic number : %d " % len(self.proto_classes)
		print "calculate_proto_classes done"
	...
	def compare_queries(self, testing=True):
		return_value = []
		print "compare_queries number : %d " % len(self.queries)
		count = 0
		for query in self.queries:
			count +=1
			if count%20==0:
